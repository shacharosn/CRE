...
SET_OF_ALL_TYPES = {('PERSON', 'STATE_OR_PROVINCE'), ('ORGANIZATION', 'ORGANIZATION'), ('ORGANIZATION', 'DATE'),
                    ...
                    ('PERSON', 'CAUSE_OF_DEATH'), ('ORGANIZATION', 'PERSON'), ('ORGANIZATION', 'STATE_OR_PROVINCE')}
...
def generator_model_input(indxs_subject, indxs_object, wiki_ner_list_all, toks, tacred_id, et_of_subj, et_of_obj, rel, span_of_subj_obj):
    global ID_COUNTER
    ID_COUNTER += 1

    subj_span = toks[indxs_subject[0]:indxs_subject[1] + 1]
    obj_span = toks[indxs_object[0]:indxs_object[1] + 1]

    gold = rel if span_of_subj_obj == [subj_span, obj_span] else 'NA'

    new_sample = {'id': tacred_id + "_" + et_of_subj + "_" + et_of_obj + "_" + str(ID_COUNTER), 'docid': 2,
                  'relation': rel, 'token': toks,
                  ...
                  'obj_type': et_of_obj, 'gold_label': gold}
                ...
                for j, et_obj in enumerate(ents_types_list):
                    if et_obj == curr_ents[1] and i != j:
                        ...
                        the_new_model_inputs = generator_model_input(indxs_subject, indxs_object,
                                                                     model_samp['stanford_ner'],
                                                                     model_samp['token'], model_samp['id'], et_subj,
                                                                     et_obj, model_samp['relation'], span_of_subj_obj)
...
with open("../span_bert/SpanBERT/permut_stanford_ner_pure_exlusive_pred_tacred_DEV_and_TEST/data/json/test.json", "w") as f:
